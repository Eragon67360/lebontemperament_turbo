...
import { CustomAudioPlayer } from "./CustomAudioPlayer"; // Import your new component
...
const AudioMemories = ({ audioMemories }: AudioMemoriesProps) => {
  ...
  const { scrollY } = useScroll(); // Note: useScroll is from framer-motion now
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 md:gap-8">
          {audioMemories.map((memory, index) => (
            <motion.div
              ...
              className="group flex flex-col overflow-hidden rounded-xl border border-slate-200/80 bg-white/30 p-6 backdrop-blur-md dark:border-slate-800/50 dark:bg-slate-900/30"
            >
              <div className="mb-4 flex items-start justify-between">
                <div>
                  <h3 className="mb-1 text-xl font-medium text-slate-900 dark:text-white">
                    {memory.title}
                  </h3>
              ...
              {/* Use your new custom audio player */}
