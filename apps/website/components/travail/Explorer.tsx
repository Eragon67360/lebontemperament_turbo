...
import { FaFile, FaFolder, FaMusic, FaPlus, FaRegFilePdf } from "react-icons/fa";
...
const Explorer: FC<ExplorerProps> = ({ initialFolderId }) => {
    ...
    const fetchData = async (folderId: string) => {
        ...
        try {
            ...
            const fetchedFolders = data.filter((file: { type: string; }) => file.type === 'folder');
            const fetchedFiles = data.filter((file: { type: string; }) => file.type === 'file');
    ...
    const handleAddElement = async (files: File[]) => {
        try {
            ...
            if (response.ok) {
                ...
                if (folderId)
                    fetchData(folderId); // Refresh the folder contents
            } else {
                toast.error('Une erreur est survenue lors de l\'export! Veuillez contacter Thomas Moser')
    ...
    const handleConfirmDelete = async () => {
        if (fileToDelete) {
            try {
                const response = await fetch(`/api/drive/files?fileID=${fileToDelete.id}`, {
                    method: 'DELETE',
                });
                if (response.ok) {
                    ...
                    toast.error('Une erreur est survenue lors de la suppression! Veuillez contacter Thomas Moser')
        ...
        <div>
            <div className='flex gap-2 items-center'>
                {folderStack.length > 1 && (
                    <button onClick={handleBackClick} className='flex px-4 py-3 border border-black/10 bg-white rounded-lg gap-4 hover:opacity-70 hover:scale-105 cursor-pointer transition-all duration-200'>
                        <IoArrowBack />
                    </button>
                )}
                <FcFolder size={24} />
                ...
                {isEditMode && (
                    <Button className='bg-transparent border border-black/50 rounded-lg hover:opacity-10' onClick={handleAddClick} startContent={
                        <FaPlus />
                    }>
                ...
                {!isEditMode && (
                    <Button className='bg-transparent border border-black/50 rounded-lg hover:opacity-10' onClick={handleEnterEditMode} startContent={
                        <MdModeEdit />
                    }>
                        Éditer le contenu
                    </Button>
                ...
                {isEditMode && (
                    <Button className='bg-transparent border border-black/50 rounded-lg hover:opacity-10' onClick={handleQuitEditMode} startContent={
                        <IoArrowUndoSharp />
                    }>
            ...
            <div className='flex flex-wrap gap-x-8 gap-y-4 w-full mt-6'>
                {
                    loading ? (<p className='font-bold text-lg italic'>Recherche en cours...</p>) : (
                        <>
                            {
                                folders.length === 0 ? (
                                    <p className='font-bold text-lg italic'>Pas de dossiers pour le moment ! </p>
                                ) : (
                                    <>
                                        {folders.map((folder) => (
                    ...
                    (
                        <>
                            {individualFiles.length === 0 ? (
                                <p className='font-bold text-lg italic'>Pas de fichiers pour le moment !</p>
                            ) : (
                                <>
