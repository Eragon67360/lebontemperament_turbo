...
export const YoutubeVideos = () => {
  ...
  return (
    <>
      {loading ? <div className='px-8 max-w-[1440px] w-full flex flex-col mb-32'>
        ...
        <>
          {
            videos.map((video) => (
              <div key={video.id} className='my-8 flex flex-col lg:flex-row gap-0 lg:gap-8'>
                ...
                <div className='flex flex-col pt-8'>
                  ...
                  <h2 className='text-sm md:text-base lg:text-lg font-bold mt-8'>
                    Auteur/Compositeur : <span className='font-normal'>{video.composer}</span>
                  </h2>
                  <h2 className='text-sm md:text-base lg:text-lg font-bold'>
                    Date : <span className='font-normal'>
                      {new Date(video.performance_date).toLocaleDateString('fr-FR')}
                    </span>
                  </h2>
                  <h2 className='text-sm md:text-base lg:text-lg font-bold'>
                  ...
                  {video.soloists && video.soloists.length > 0 && (
                    <h2 className='text-sm md:text-base lg:text-lg font-bold'>
                      Solistes : <span className='font-normal'>{video.soloists.join(', ')}</span>
                    </h2>
                  )}
                </div>
              </div>
