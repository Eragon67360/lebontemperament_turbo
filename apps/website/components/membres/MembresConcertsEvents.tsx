...
const MembresConcertsEvents = () => {
    ...
    const formatEventDate = (event: Event) => {
        const dateFrom = format(new Date(event.date_from), 'dd MMMM yyyy', { locale: fr });
        if (event.date_to) {
            const dateTo = format(new Date(event.date_to), 'dd MMMM yyyy', { locale: fr });
    ...
    return (
        <div className="px-8 max-w-[1440px] w-full flex flex-col">
            <div className='py-8'>
                <h1 className="text-title text-primary/50 font-light leading-none">Nos</h1>
                <h2 className="text-title text-[#333] font-bold leading-none">Prochains concerts</h2>
                <hr className="my-8" />
                        ...
                        {concerts.map((concert, index) => (
                            ...
                            >
                                <h3 className="font-semibold text-lg text-gray-800 mb-4">
                                    {concert.name || `Concert du ${format(new Date(concert.date), 'dd MMMM yyyy', { locale: fr })}`}
                                </h3>
                                ...
                                <div className="space-y-3 text-sm text-gray-600">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span>
                                            {format(new Date(concert.date), 'dd MMMM yyyy', { locale: fr })}
                                        </span>
                                    </div>
            ...
            <div className='py-8'>
                <h1 className="text-title text-primary/50 font-light leading-none">Nos</h1>
                <h2 className="text-title text-[#333] font-bold leading-none">Événements à venir</h2>
                <hr className="my-8" />
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {events.map((event, index) => (
                            ...
                            >
                                ...
                                {event.link && (
                                    <a href={event.link} target='_blank' rel='noreferrer' className="flex gap-2 px-3 py-1 mt-4 w-fit items-center text-xs font-medium bg-gray-100 text-gray-600 rounded-full">
            ...
            <div className='py-8'>
                <h1 className="text-title text-primary/50 font-light leading-none">Concert</h1>
                <h2 className="text-title text-[#333] font-bold leading-none">Anniversaire</h2>
                <hr className="my-8" />
                <h2 className='text-base md:text-lg lg:text-xl'>
                    L&apos;album ci-dessous est un enregistrement d&apos;un concert anniversaire pour les 20 ans du Bon Tempérament
                </h2>
                <div className='mt-8'>
                    <MusicList />
                </div>
