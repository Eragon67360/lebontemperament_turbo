...
const ConcertsClient = () => {
    ...
    const [selectedConcertImage, setSelectedConcertImage] = useState<string | null>(null)
                                    ...
                                    {concert.affiche ? (
                                        <Tooltip content="Cliquez pour agrandir">
                                            <Image
                                                ...
                                                width={400}
                                                height={200}
                                                onClick={() => handleImageClick(concert.affiche as string)}
                                                className="w-full h-48 object-cover cursor-pointer"
                                            />
                                        </Tooltip>
                                    ) : (
                                        <div className="w-full h-48 bg-gray-200 flex items-center justify-center">
                                            <span className="text-gray-400">Aucune affiche disponible</span>
                                        </div>
                                    )}

                                    <div className="p-4 space-y-3">
                                        <h4 className="font-semibold text-lg text-gray-800 line-clamp-2">
                                            {concert.name || `Concert du ${format(new Date(concert.date), 'dd MMMM yyyy', { locale: fr })} à ${concert.place}`}
                                        </h4>
                                ...
                                >
                                    <div className="p-4 space-y-3">
                                        <div className="flex justify-between items-start">
                                            ...
                                            <span className="inline-block px-3 py-1 text-xs font-medium bg-primary/10 text-primary rounded-full">
                                                {event.event_type === 'concert' ? 'Concert' :
                                                    event.event_type === 'repetition' ? 'Répétition' :
                                                        event.event_type === 'vente' ? 'Vente' : 'Autre'}
                                            </span>
                                        </div>
                                        ...
                                        <div className="space-y-2 text-sm text-gray-600">
                                            <div className="flex items-center gap-2">
                                                <IoCalendarClear className="text-primary" />
                                                <span>
                                                    {format(new Date(event.date_from), 'dd MMMM yyyy', { locale: fr })}
                                                    {event.date_to && ` au ${format(new Date(event.date_to), 'dd MMMM yyyy', { locale: fr })}`}
                                                </span>
                                            </div>
                        ...
                        >
                            ...
                            >
                                <CloudinaryImage
                                    ...
                                    width={1000}
                                    height={500} rounded={RoundedSize.NONE} />
                            </Link>
