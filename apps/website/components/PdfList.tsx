...
const PdfList: React.FC<PdfListProps> = ({ jsonFileName, context }) => {
    ...
    const getResultForContext = (context: string) => {
        const contextResult = contextTitre[context] + ' ' + contextPreposition[context];
    ...
    return (
        <div className='mt-8 flex flex-wrap gap-6'>
            {pdfFiles.map((file: PdfFile, index: number) => (
                <Link href={`/pdf/${context}/${file.name}`} key={index} target="_blank" rel="noopener" className='p-2 lg:p-4 rounded-lg bg-primary text-white flex gap-4 items-center hover:bg-[#18858ba7] text-xs md:text-sm'>
                    <FaRegFilePdf /> <span>{getResultForContext(context)}{formatDate(file.date)}</span>
                </Link>
            ))}
        </div>
    );
